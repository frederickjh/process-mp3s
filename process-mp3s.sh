...
# Check the version of eyeD3. We have only tested with version 0.6.18. The next version 0.7.0 breaks the API and may require a rewrite.
    eyed3version=($(eyeD3 --version 2>/dev/null | head -n 1| awk '{print $2}'| awk -F "." '{print $1}'' ''{print $2}'' ''{print $3}'))
...
function setid3tags(){
  ...
  eyeD3 --log-level error --user-url-frame="WXXX:" --remove-all-images "${unsanitizedfilename}" 
  # Check if we are including the image tag or not.
  if [ "${addimagetag}" = "yes" ]; then
  eyeD3 --log-level error --artist "${id3artist}" --album "${id3album}" --title "${id3title}" --track ${id3track} --track-total ${id3combinetracktotal} --publisher "${id3publisher}" --text-frame="TCOP:${id3copyright}" --release-year ${id3year} --recording-date "${id3recordingdate}" --orig-release-date "${id3recordingdate}" --release-date "${id3recordingdate}"  --encoding-date "${id3recordingdate}" --tagging-date "${id3recordingdate}" --user-url-frame="WXXX:${id3url}" "--add-image=${id3image}:FRONT_COVER:Regichile Logo" "${unsanitizedfilename}"
  elif [ "${addimagetag}" = "no" ]; then
    eyeD3 --log-level error --artist "${id3artist}" --album "${id3album}" --title "${id3title}" --track ${id3track} --track-total ${id3combinetracktotal} --publisher "${id3publisher}" --text-frame="TCOP:${id3copyright}" --release-year ${id3year} --recording-date "${id3recordingdate}" --orig-release-date "${id3recordingdate}" --release-date "${id3recordingdate}" --encoding-date "${id3recordingdate}" --tagging-date "${id3recordingdate}" --user-url-frame="WOAR:${id3url}" "${unsanitizedfilename}"
  fi
}
